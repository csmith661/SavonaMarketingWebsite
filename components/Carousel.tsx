    ...
    <div className='relative w-full h-80'>
        <div className="embla overflow-hidden" ref={emblaRef}>
            <div className='embla_container flex'>
                {imagesArray.map((image, index)=>(<div key={index} className='empla__slide flex-[0_0_100%]'><><Image className="h-80" quality={75} loading="eager" src={image} alt="portfolio example"/></></div>))}
            </div>
        </div>
        <div className="selectorBar h-1 absolute w-full bottom-1 flex items-center justify-center gap-4">
            {imagesArray.map((value, index)=>(<div key={index} className={`h-full w-8 bg-${index === selectedImageIndex ? 'primary':'secondary'}`}> </div>))}
        </div>
    </div>
